# coding=utf-8
# reference: http://www.cnblogs.com/zhangchaoyang/articles/5636048.html
...
class OneDimClassifier(object):
    ...
    def train(self, w):
        ...
        '''
        # 将x,y和w进行绑定，分别对应：样本值，标签（-1，1），误差统计权重
        sx = np.concatenate(
            (self.x, self.y.reshape((self.y.shape[0], 1)), w.reshape(
                (w.shape[0], 1))),
            axis=1)
        # 样本排序
        ...
        for i in range(1, len(sx)):
            ...
            if sx[i][1] != prevY:
                ...
                d = True
                for j in range(i):
                     if sx[j][1] != 1: # 输出结果评价对比 y
                        err += sx[j][2] # 误差统计时统计 w
                ...
                if err > 0.5:
                    ...
                    d = False
                if err < minErr:
                    self.split = (sx[i][0] + sx[i - 1][0]) / 2 # 弱分类器判别阈值由 x 计算，split 为弱分类器中sign()的判别阈值
                    minErr = err
                    self.direct = d
            prevY = sx[i][1]
...
class Adboost(object):
    def __init__(self, x, y, WeakClassifier, M):
        ...
        # 初始化每个样本的权重
        self.w = np.array(
            [1.0 / self.x.shape[0] for i in range(self.x.shape[0])])
        # 弱分类器的构建器
    ...
    def train(self):
        ...
        for i in range(self.M):
            ...
            if errnum == 0:
                print(
                    self.Q,
                    "week classifiers is enough to make the error of train set to zero"
                )
                break
        # 返回在训练集上的错误率
